...
class EvergreenPatronAPI(BasicAuthAuthenticator, XMLParser):
    ...
    log = logging.getLogger("Evergreen Patron API")
    ...
    def from_config(cls):
        config = Configuration.integration(
            Configuration.EVERGREEN_INTEGRATION, required=True)
        ...
        if not host:
            cls.log.info("No Evergreen Patron client configured.")
            return None
    ...
    def pintest(self, barcode, pin):
        ...
        seed = seed_response.text[seed_response.text.find("[")+2:seed_response.text.find("]")-1]

	m_pass = hashlib.md5()
        m_pass.update(pin)
        md5_password = m_pass.hexdigest()

        m_seed = hashlib.md5()
        m_seed.update(seed+md5_password)
        md5_password_and_seed = m_seed.hexdigest()

        encoded_json = urllib.quote_plus('{"password":"' + md5_password_and_seed + '","type":"opac","org":0,"username":"' + barcode + '"}')
        request_url = self.root + "osrf-gateway-v1?service=open-ils.auth&method=open-ils.auth.authenticate.complete&param=%s" % encoded_json
        
        login_response = requests.get(request_url, verify=False)
        
	reply_text = login_response.text
        if login_response.status_code != 200:
            msg = "Got unexpected response code %d. Content: %s" % (
                login_response.status_code, login_response.content
            )
            raise Exception(msg)
        elif "LOGIN_FAILED" in reply_text:
            msg = "Login failed. Content: %s" % (
...
class DummyEvergreenPatronAPI(EvergreenPatronAPI):
...
AuthenticationAPI = EvergreenPatronAPI
